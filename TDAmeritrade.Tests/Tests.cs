...
namespace TDAmeritrade.Tests
{
    public class Tests
    {
        ...
        public void TestTimeConverter()
        {
            ...
            Assert.That(time1.Minute == 00);
            Assert.That(time1.Hour == 4);
            Assert.That(time1.Day == 25);
            Assert.That(time1.Month == 5);
            Assert.That(time1.Year == 2016);
            Assert.That(time1.DayOfWeek == System.DayOfWeek.Wednesday);

            var time2 = new DateTime(2016, 5, 25, 4, 0, 0, 0, DateTimeKind.Utc);
            var time3 = TDHelpers.ToEST(time2);
            Assert.That(time3.Hour == 0);

            var stamp2 = time2.ToUnixTimeSeconds();
            var stamp3 = TDHelpers.UnixSecondsToMiliseconds(stamp2);
            Assert.That(stamp3 == 1464148800000);
        }
        ...
        public async Task TestOptionChain()
        {
            ...
            Assert.That(chain.callExpDateMap.Count > 0);
        }
        ...
        public async Task TestMarketHours()
        {
            ...
            Assert.That(hours.marketType == "EQUITY");
        }
        ...
        public void TestCandleConsolidate()
        {
            ...
            Assert.That(merge1.Length == 3);
            var merge2 = TDHelpers.ConsolidateByTotalCount(array, 5);
            Assert.That(merge2.Length == 5);
            var merge3 = TDHelpers.ConsolidateByTotalCount(array, 1);
            Assert.That(merge3.Length == 1);

            Assert.That(merge3[0].open == 0);
            Assert.That(merge3[0].high == 8);
            Assert.That(merge3[0].low == 0);
            Assert.That(merge3[0].close == 8);
            Assert.That(merge3[0].volume == 36);
        }
        ...
        public async Task TestTDQuoteClient_Option()
        {
            ...
            Assert.That(data.symbol == "SPY_231215C500");
        }
        ...
        public async Task TestTDPrincipalClient()
        {
            ...
            Assert.That(!string.IsNullOrEmpty(data.accessLevel));
        }
        ...
        public async Task TestRealtimeStream()
        {
            ...
            using (var socket = new TDAmeritradeStreamClient(client))
            {
                ...
                Assert.That(socket.IsConnected);
